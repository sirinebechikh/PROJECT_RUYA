...
@Component({
  selector: 'app-effet40',
  templateUrl: './effet40.html',
  styleUrls: ['./effet40.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule],
})
export class Effet40Component implements OnInit, OnDestroy {
  fichiers: Fichier[] = [];
  ...
  ngOnInit() {
    this.subscription = this.ajouterFichierService.fichiers$.subscribe(data => {
      this.fichiers = (data || []).filter(f => f.typeFichier === 'effet' && f.codeValeur === '40');
      this.applyFilters();
  ...
  modifierFichier(fichier: Fichier) {
    this.fichierSelectionne = {...fichier}; // clone pour éviter modif directe
